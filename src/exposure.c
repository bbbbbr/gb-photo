...
// can't return a point to data that is banked since it's bank won't remain active
void GAINS_CAPTION_SPRINTF           (const uint8_t * format, uint8_t index) BANKED { sprintf(text_buffer, format, gains[index].caption);           }
void DITHER_PATTERNS_CAPTION_SPRINTF (const uint8_t * format, uint8_t index) BANKED { sprintf(text_buffer, format, dither_patterns[index].caption); }
void ZERO_POINTS_CAPTION_SPRINTF     (const uint8_t * format, uint8_t index) BANKED { sprintf(text_buffer, format, zero_points[index].caption);     }
void VOLTAGE_REFS_CAPTION_SPRINTF    (const uint8_t * format, uint8_t index) BANKED { sprintf(text_buffer, format, voltage_refs[index].caption);    }
void EDGE_RATIOS_CAPTION_SPRINTF     (const uint8_t * format, uint8_t index) BANKED { sprintf(text_buffer, format, edge_ratios[index].caption);     }
void EDGE_OPERATIONS_CAPTION_SPRINTF (const uint8_t * format, uint8_t index) BANKED { sprintf(text_buffer, format, edge_operations[index].caption); }
